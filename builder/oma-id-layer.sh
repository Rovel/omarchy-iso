...
# The smoke hook (generated by build-iso.sh) only fires when this marker exists.
if [[ -n "${OMA_ID_SMOKE:-}" ]]; then
  touch "$AIROOTFS/opt/oma-id/.oma-id-smoke"
fi
# STEP 0 (school/work choice + URL + machine name + keygen + reservation +
# possession): SOURCED by the quattro configurator at its top; personal
# installs never see it. installer-choice.sh is the standalone pre-quattro
# variant, kept for reference/CI.
install -D -m 0755 "$src/tests/iso-smoke/oma-id-step0.sh" "$AIROOTFS/opt/oma-id/bin/oma-id-step0.sh"
